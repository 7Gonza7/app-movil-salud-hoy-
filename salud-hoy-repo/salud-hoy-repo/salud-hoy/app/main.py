...
# Importar el módulo de base de datos (usando imports absolutos del paquete)
...
class SaludHoyApp(MDApp):
    ...
    def animate_register_screen(self):
        """Anima la pantalla de registro con fade in suave"""
        try:
            reg_card = self.root.ids.reg_card
            reg_card.opacity = 0
            from kivy.animation import Animation
            from kivy.clock import Clock
            def animate_card(dt):
                anim = Animation(opacity=1, d=0.4, t="out_cubic")
                anim.start(reg_card)
            Clock.schedule_once(animate_card, 0.1)
        except Exception as e:
            print(f"[ERROR] Error en animación de registro: {e}")
